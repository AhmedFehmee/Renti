...
public class ForgetPassword extends AppCompatActivity {
    ...
    private void callPasswordService() {
        ...
        String url = "http://2.extra4it.net/tajeree/api/sendmail";
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (status.equals("1")) {
                                Intent intent = new Intent(getApplicationContext(), ForgetPasswordConfirmation.class);
                                intent.putExtra("email" , email.getText().toString());
                                startActivity(intent);
                                Snackbar.make(getWindow().getDecorView().getRootView(),
                                        getString(R.string.forget_password_mail), Snackbar.LENGTH_LONG)
                                        .setAction("Action", null).show();
                            } else if (status.equals("0")) {
                                Snackbar.make(getWindow().getDecorView().getRootView(),
                                        getString(R.string.invalid_email), Snackbar.LENGTH_LONG)
                                        .setAction("Action", null).show();
                            } else {
                                Snackbar.make(getWindow().getDecorView().getRootView(),
